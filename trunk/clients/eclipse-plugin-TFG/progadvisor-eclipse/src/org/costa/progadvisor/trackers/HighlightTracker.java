...
public class HighlightTracker extends CommandTracker{
	...
	public static final String MARKER_HIGHLIGHT = "org.costa.progadvisor.marker.highlight";
	public static final String MARKER_HIGHLIGHT_WARN = "org.costa.progadvisor.marker.highlightwarn";
	...
	public HighlightTracker(HighlightCommand command) {
		super(command);
	}
	...
	public void track() throws CostabsException {
		...
		for(int i = 0; i < getLinesInt().length; i ++) {
			try {
				...
				if(getLevel() == null)
					marker = this.getiFile().createMarker(getMarker("info"));
				else
					marker = this.getiFile().createMarker(getMarker(getLevel()));
